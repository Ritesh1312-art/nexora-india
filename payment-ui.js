(()=>{
 ...
 async function client(){if(clientPromise)return clientPromise;clientPromise=fetch("/api/config").then(r=>r.json()).then(c=>c.url&&c.key?supabase.createClient(c.url,c.key):null);return clientPromise}
 async function enhance(){
  ...
  for(const panel of panels){
   ...
   const orderNumber=bold.textContent.trim();if(!orderNumber)continue;panel.dataset.utrEnhanced="1";
