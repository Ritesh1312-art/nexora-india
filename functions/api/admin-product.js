...
async function saveProduct(env,b){
 ...
 if(id){const old=await supabase(env,`products?id=eq.${encodeURIComponent(id)}&select=source,supplier_id,source_product_id,source_sku`);const rows=await old.json().catch(()=>null);if(!old.ok||!Array.isArray(rows)||!rows[0])return json({error:'Product not found'},404);sourceFields={source:rows[0].source||'MANUAL',supplier_id:rows[0].supplier_id||null,source_product_id:rows[0].source_product_id||null,source_sku:rows[0].source_sku||null};}
