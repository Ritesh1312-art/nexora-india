...
export async function onRequestPost({request,env}){
 try{
  ...
  if(!env.SUPABASE_SERVICE_ROLE_KEY)return json({error:"SUPABASE_SERVICE_ROLE_KEY is not configured in Cloudflare Pages Functions environment"},500);
  ...
  if(b.action==="stats"){
   ...
   if(!Array.isArray(ps)||!Array.isArray(us)||!Array.isArray(os))return json({error:"Supabase returned an error",details:{products:ps,users:us,orders:os}},502);
   return json({products:ps.length,users:us.length,orders:os.length,sales:os.reduce((s,x)=>s+Number(x.total_amount||0),0)});
  }
