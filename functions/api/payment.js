...
export async function onRequestPost({request,env}){try{
 ...
 const stockItems=items.map(x=>({product_id:x.product_id,quantity:Number(x.quantity||1)}));let claimedReleased=false;
 if(order.stock_released){
  // Claim the released state before reserving. Only one concurrent UTR submission
  // can transition stock_released=true -> false, preventing double reservation.
