...
async function releaseOrderStock(env,orderId,currentStatus){
 ...
 const ir=await supabase(env,`order_items?order_id=eq.${encodeURIComponent(orderId)}&select=product_id,variant_id,quantity`);const items=await ir.json();
...
export async function onRequestPost({request,env}){
 try{
  ...
  if(RELEASE.has(next)&&rows[0].stock_reserved&&!rows[0].stock_released){
   const result=await releaseOrderStock(env,orderId,current);
