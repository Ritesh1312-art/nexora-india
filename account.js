...
(function(){
  ...
  async function switchTab(tab,parentRequest=tabRequest){
    ...
    const buttons=document.querySelectorAll('.account-nav button');buttons.forEach(b=>b.classList.toggle('active',b.dataset.tab===tab));
    const panel=document.querySelector('#accountPanel');if(!panel)return;
    const request=++tabRequest;
    if(tab==='profile')return renderProfile(panel,request);
    if(tab==='orders')return renderOrders(panel,request);
    return renderSecurity(panel,request);
  }
  ...
  async function renderProfile(panel,request){
    ...
    const sb=client();
    try{
      if(!sb)throw new Error('Account database client is not ready.');
      const r=await sb.from('profiles').select('full_name,phone,address_line1,address_line2,city,state,pincode').eq('id',user().id).maybeSingle();
      if(r.error)loadError=r.error.message;else p=r.data||{};
    }catch(e){loadError=e.message||'Unable to load profile.'}
    if(request!==tabRequest||!user())return;
  ...
  async function saveProfile(){
    ...
    const sb=client(),msg=document.querySelector('#accountMsg');if(!sb){if(msg)msg.innerHTML='<div class="notice error">Database connection is not ready. Please try again.</div>';return}
    ...
    try{
      let r=await sb.from('profiles').update(payload).eq('id',user().id).select().maybeSingle();
      if(r.error)throw r.error;
      if(!r.data){const up=await sb.from('profiles').upsert({id:user().id,...payload},{onConflict:'id'}).select().maybeSingle();if(up.error)throw up.error;}
      if(msg)msg.innerHTML='<div class="notice success">Profile and address updated successfully.</div>';
    }catch(e){if(msg)msg.innerHTML=`<div class="notice error">${esc(e.message||'Could not save profile.')}</div>`}
    finally{b.disabled=false;b.textContent='Save changes'}
  }

  async function renderOrders(panel,request){
    panel.innerHTML='<div class="account-card"><div class="muted">Loading orders…</div></div>';
    const sb=client();
    try{
      if(!sb)throw new Error('Account database client is not ready.');
      const r=await sb.from('orders').select('id,order_number,created_at,total_amount,order_status,payment_status,order_items(product_name,quantity,unit_selling_price)').eq('user_id',user().id).order('created_at',{ascending:false});
      if(r.error)throw r.error;
      if(request!==tabRequest||!user())return;
      const orders=r.data||[];
    ...
    }catch(e){if(request===tabRequest)panel.innerHTML=`<div class="account-card"><h2>My Orders</h2><div class="notice error">Could not load orders: ${esc(e.message||'Unknown error')}</div></div>`}
  }

  function renderSecurity(panel,request){
    if(request!==tabRequest)return;
    panel.innerHTML='<div class="account-card"><h2>Security</h2><p class="muted">Use password recovery to verify your email and create a new password.</p><div class="account-actions"><a class="btn secondary" href="#/forgot">Reset password</a></div></div>';
  }
