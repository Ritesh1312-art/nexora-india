...
(function(){
  ...
  // Store navigation must never clear an authenticated session. Only logout clears it.
  function store(){if(location.hash!=='#/')location.hash='#/';syncNav(!!window.session);setTimeout(()=>{try{if(typeof window.renderHome==='function')window.renderHome();else window.route?.()}catch(e){console.error('Nexora Store render:',e)}},0);}
  ...
  function handleNavigation(sb,e){
    ...
    if(authHashes.has(target)&&logged){store();return;}
  ...
  async function boot(){
    try{
      ...
      sb.auth.onAuthStateChange((_event,s)=>{window.session=s||null;window.sb=sb;syncNav(!!s);setTimeout(()=>{if(!s)store();else if(authHashes.has(location.hash))store();else enforce(sb).catch(()=>{})},0)});
